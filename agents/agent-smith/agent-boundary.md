...
| Approved charter content | Input | Charter specification provided by human or agent-architect; defines mission and authority of the new agent |
| `agents/agent-smith/templates/` | Input | Source template files read by agent-smith to generate new agent artifacts |
| `governance/constitution.md` | Input | Read to ensure created agents comply with workspace principles |
| `agents/{new-agent}/agent-charter.md` | Output | Charter file generated by agent-smith for the new agent |
| `agents/{new-agent}/templates/` | Output | Prompt templates generated by agent-smith for the new agent |
| `agents/{new-agent}/` | Output | Populated agent folder; agent-smith writes all artifacts into it |
